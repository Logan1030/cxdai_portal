...
import com.cxdai.portal.curAccount.vo.CurAccountVo;
...
public class MyAccountReportServiceImpl implements MyAccountReportService {
	...
	public Map<String, BigDecimal> queryUserAccountDetail(Integer memberId) throws Exception {
		...
		BigDecimal netMoney = yesInterstTotal.add(receiveInterest)
				.add(awardTotal).add(stockMoney).add(transferDiff).add(curInterestTotal).add(fixInvestYesTotal);
		...
		/*
		 * 总的费用支出(新) = 借款利息支出 + 充值费用支出 + 借款管理费支出 + 利息管理费支出 +提现费用支出 + 转可提费用支出
		 * +罚息支出 + VIP费用支出 + 转让管理费支出+债权认购支出
		 */
		BigDecimal payTotal = totalDetailMoney.add(payLateInterest).add(cashCost).add(havaPayInterest).add(totalFeeMoney).add(transferManageFee).add(firstTransferManageFee).add(subscribeTransferDiff);
