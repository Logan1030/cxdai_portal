...
import com.cxdai.portal.account.vo.RiskMarginVo;
...
public interface RiskMarginMapper {
	...
	@Select("SELECT SUM(yest) from "
			+ "(SELECT IFNULL(sum(ACCOUNT),0)yest from rocky_borrow where `STATUS`in(4,5,42) and DATEDIFF(CURDATE(),FROM_UNIXTIME(SUCCESS_TIME,'%Y-%m-%d'))=1 "
