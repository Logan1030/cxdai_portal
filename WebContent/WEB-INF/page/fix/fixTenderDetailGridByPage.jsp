     ...
     <td class="tdtitl04" style="overflow:visible;">
         <c:if test="${loginMember.userId == tenderDetail.userId}">
         	<c:if test="${tenderDetail.redMoney>0}">
     	 		<a href="javascript:void(0);" class="listtishi"><img src="${path}/images/v5/hb.png" style="padding-top:15px;"/><p class="displaywm1 f14" >使用红包:<fmt:formatNumber value="${tenderDetail.redMoney }" pattern="#,##0.##" />元</p></a>
         	</c:if>
			<a style="color: blue;"  href="${path}/accountdetail/show.html?userName=${portal:encode(portal:encode(tenderDetail.userNameEncrypt))}" target="_blank">${tenderDetail.userName} </a>
		 </c:if> 
		 <c:if test="${loginMember.userId != tenderDetail.userId}"><a href="${path}/accountdetail/show.html?userName=${portal:encode(portal:encode(tenderDetail.userNameEncrypt))}" target="_blank">${tenderDetail.userNameSecret}</a></c:if>
     </td>
     <td class="tdtitl05">
