...
<div class="invest-cent"> 
   ...
   <span class="zi1" >
     <font color="red">
     <c:if test="${search_type==null or search_type == 'week'}"><span>本周内</span></c:if>
     <c:if test="${search_type == 'month'}"><span>本月内</span></c:if>
     <c:if test="${search_type == 'quarter'}"><span>本季度内</span></c:if>
     <c:if test="${search_type == 'year'}"><span>本年度内</span></c:if>
     </font>应归还款额：<font color="red">￥<fmt:formatNumber value="${repaymentChartVoMap.repaymentAccount}" pattern="#,##0.00"/></font>(其中已归还：<font color="red">￥<fmt:formatNumber value="${repaymentChartVoMap.yesRepaymentAccount}" pattern="#,#00.00"/></font>，未归还：<font color="red">￥<fmt:formatNumber value="${repaymentChartVoMap.noRepaymentAccount}" pattern="#,#00.00"/></font>)
   </span>
</div>
