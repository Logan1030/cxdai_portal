...
<%@ page import = "com.cxdaiweb.statics.Enumeration" %>
...
function tender_borrow(){
	...
	if(msg!=""){
		...
		if(layer.confirm("确定要投标吗？",function(){
			...
			$.post("${basePath}/newcxdai/investment/invest/tender/${borrow.id}.html",{
				...
				if(data=="OK,Rocky.J!"){
					...
					window.open("${path}/newcxdai/investment/invest/toInvest.html?id=${borrow.id}","_parent");
				}else{
					layer.alert(data);
					var random = Math.random();
					document.getElementById("randImage").src = "${basePath}/random.jsp?" + random;
...
<input type="hidden" id="effectiveTenderMoney" value="${effectiveTenderMoney }"/>
	<div id="tc_main">
		...
		<div class="tc_main_d"> <span class="tc_main_e"> 您的可用余额：¥ </span> <span class="tc_main_b" id="use_money"><fmt:formatNumber value="${account.use_money }" pattern="#,#00.00"/><a href="${path}/newcxdai/account/topup/toTopupMain/topup.html" target="_parent">&nbsp;&nbsp;我要充值</a></span>        </div>
		...
		<div class="tc_main_a">
			<a  id="tenderBorrow" href="javascript:tender_borrow();"><img src="${basePath}/cxdaiweb/images/tc_tj.jpg" width="77" height="33" border="0" /></a>
			&nbsp;&nbsp;
			<!--  <a  id="tenderClose" href="javascript:tender_close();"><img src="${basePath}/cxdaiweb/images/tc_tj.jpg" width="77" height="33" border="0" /></a>-->
		</div>
		<script type="text/javascript">
