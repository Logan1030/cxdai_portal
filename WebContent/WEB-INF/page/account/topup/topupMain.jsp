...
//进入线下充值页面
function topdownFun(){
	$.post("${basePath}/newcxdai/account/topup/IsJudgeBank.html", {
	}, function(data) {
		if(data == 'fail'){
		...
		if(data=='false'){
			...
			$.post("${basePath}/newcxdai/account/topup/toTopdown.html", {
			}, function(data) {
				setlicss();
...
function paymentDetail(pageNum){
	...
	var type = $("#type").val();
	$.post("${basePath}/newcxdai/myaccount/paymentDetails/"+pageNum+"/10.html", {
...
function paymentDetailQuery(){
	...
	var endTime = $("#endTime").val();
	var type = $("#type").val();
	$.post("${basePath}/newcxdai/myaccount/paymentDetails/1/10.html", 
...
//进入银行卡管理页面
function toBankCard(){
	/*$.post("${basePath}/newcxdai/account/topup/IsJudgePayPassword.html", {
	}, function(data) {
		if(data=='false'){
			layer.alert("您的初始交易密码为空，请先设置交易密码");
			$.layer({
				...
				close : function(index){
					...
					$.post("${basePath}/newcxdai/account/topup/IsJudgePayPassword.html", {
						...
						if(data=='true'){
							window.open("${path}/newcxdai/account/topup/toTopupMain/toChinabank.html",'_parent');
						}
					});
				}
			});
		}else if(data=='true'){
			$.post("${basePath}/newcxdai/account/topup/toBankManger.html", {
			}, function(data) {
				setlicss();
